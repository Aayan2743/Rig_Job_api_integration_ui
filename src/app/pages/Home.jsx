...
const fetchFeaturedJobs = async () => {
  try {
    ...
    const res = await api.get("/companies/featured-jobs");
    ...
    console.log("Featured Jobs Error:", error.response?.data || error.message);
  } finally {
...
const fetchIndustries = async () => {
  try {
    ...
    const res = await api.get("/companies/industries-with-jobs");
    ...
    console.log("Industries Error:", error.response?.data || error.message);
  } finally {
...
const fetchTopCompanies = async () => {
  try {
    ...
    console.log("Top Companies Error:", error.response?.data || error.message);
  } finally {
  ...
  // ✅ fallback UI values
                  const colors = [
                    "bg-blue-50", "bg-emerald-50", "bg-cyan-50", "bg-purple-50",
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {/* ICON */}
                        <div className="flex items-start justify-between mb-4">
                          {/* <div className={`w-14 h-14 rounded-xl ${bg} flex items-center justify-center text-2xl`}>
                            🛢️
                          </div> */}

                          <div className={`w-14 h-14 rounded-xl overflow-hidden ${bg} flex items-center justify-center`}>
                  {category.image ? (
                    <img
                      src={category.image}
                      alt={category.name}
                    ...
                    <span className="text-xl">🛢️</span>
                        ...
                        {/* NAME */}
                        <h3 className="text-lg font-bold text-foreground mb-1 group-hover:text-primary">
                          {category.name}
                        </h3>

                        {/* COUNT (FIXED) */}
                        <p className="text-muted-foreground text-sm">
                          {(category.jobs_count || 0)} open positions
                        </p>
                        ...
                        <div className="mt-3 h-1 w-full bg-muted/40 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min((category.jobs_count || 0) / 1500 * 100, 100)}%`
                            }}
                          />
                        </div>
  ...
  <p className="text-center text-gray-400">No companies found</p>
) : (
