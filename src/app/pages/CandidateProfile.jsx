...
export function CandidateProfile() {
  ...
  const handleUpload = async () => {
    try {
      ...
      await api.post("/candidate/profile/image", formData, {
        headers: {
  ...
  const handleSave = async (field) => {
    try {
      ...
      await api.post("/candidate/profile/update", payload);
    ...
    // }
      ...
      // Optional: show generic message OR nothing
      // showError("Something went wrong"); // OR remove completely
    }
  };
  ...
  const handleViewResume = (id) => {
    window.open(`${API_URL}/api/candidate/resume/${id}/view`, "_blank");
  };
  ...
  const handleDownloadResume = async (id) => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", fileName); // ✅ correct filename with extension
